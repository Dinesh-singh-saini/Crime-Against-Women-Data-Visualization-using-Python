...
def visualize_data_by_state():
    ...
    if choice == 1:
        state_totals = data.groupby('STATE/UT').sum(numeric_only=True).sum(axis=1)
        state_totals.plot(kind='bar', figsize=(22, 18), title="Total Crimes by State/UT")
        ...
        state_data.iloc[:, 2:].T.plot(figsize=(10, 6), title="Crime Trends Comparison")
        plt.ylabel('Crimes')
        plt.show()

    elif choice == 3:
        state = input("Enter State/UT name: ")
        state_data = data[data['STATE/UT'] == state].set_index('STATE/UT')
        state_data.iloc[:, 2:].T.plot(kind='bar', figsize=(10, 6), title=f"Crime Type Breakdown in {state}")
